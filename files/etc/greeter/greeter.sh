...
if test "$content" -eq 0; then
	...
	# make sure the overview stays disabled
	touch "${disable_overview_file}"
...
# we need to wait a bit until we delete lock files because it is possible that
# the gnome extension is too slow to check for the files
sleep 5
